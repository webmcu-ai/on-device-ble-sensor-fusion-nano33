// ======================================================
// XIAO ESP32S3 (SENSE)
// ON-DEVICE BLE SENSOR FUSION - CENTRAL/CONTROLLER + ML - v08
// (on-device-ble-sensor-fusion-v08)
//
...
void myStartBLEPeripheralServer() {
  ...
  myServerBinaryChar    = svc->createCharacteristic(MY_BLE_BINARY_CHAR_UUID, NIMBLE_PROPERTY::INDICATE);
...
void myServerSendBinary(const uint8_t* raw, uint32_t totalBytes) {
  ...
  myServerBinaryChar->indicate();
  const int chunkSize = 180;
  for (uint32_t off = 0; off < totalBytes; off += chunkSize) {
...
void myActionInferOnce() {
  ...
  Serial.printf("Predicted: %s  (", myClassLabels[pred].c_str());
  for (int j = 0; j < NUM_CLASSES; j++) Serial.printf("%s=%.0f%%%s", myClassLabels[j].c_str(), myFinal_output[j] * 100.0f, j < NUM_CLASSES - 1 ? ", " : "");
  Serial.println(")");
...
void myActionQuickInferOnce() {
  ...
  Serial.printf("top: %s = %.0f%%, (", myClassLabels[pred].c_str(), myFinal_output[pred] * 100.0f);
  for (int j = 0; j < NUM_CLASSES; j++) {
    Serial.printf("%.0f%%", myFinal_output[j] * 100.0f);
    if (j < NUM_CLASSES - 1) Serial.print(", ");
  }
  Serial.println(")");
  myServerSendResult(pred, myFinal_output, NUM_CLASSES);   // [NEW v05]
...
void setup() {
  ...
  Serial.println("\n=== XIAO ESP32-S3 BLE Sensor Fusion System v08 ===");
